/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package Datas;

import java.util.Arrays;
import java.util.List;

/**
 *
 * @author VICTOR CUNHA
 */
public class Estacoes {
    public static final List<EstacaoTemperada> estacoesNorte = Arrays.asList( EstacaoTemperada.InvernoBoreal,EstacaoTemperada.OutonoBoreal,EstacaoTemperada.PrimaveraBoreal,EstacaoTemperada.VeraoBoreal);
    public static final List<EstacaoTemperada> estacoesSul = Arrays.asList( EstacaoTemperada.InvernoAustral,EstacaoTemperada.OutonoAustral,EstacaoTemperada.PrimaveraAustral,EstacaoTemperada.VeraoAustral);
}
